...
(defun and-val (x y)
	(cond ((and (not (equal x 0))(not (equal y 0)))  1)
		  (t 0)))
...
(defgeneric equal-val-tensor-scalar (t1 t2))

(defmethod equal-val-tensor-scalar ((t1 tensor) (t2 scalar))
  (let ((result ())
     (tensor-elements (slot-value t1 'elements)))
    (cond ((not (listp (car tensor-elements))) (dolist (it tensor-elements result) (setf result (append result (list (equal-val it (slot-value t2 'elements)))))))
        (t (loop for x in tensor-elements
          do (setf result (append result (list (equal-val-tensor-scalar (make-instance 'tensor :tensor-elements x) t2)))))))
    result))

(defmethod equal-val-tensor-scalar ((t2 scalar) (t1 tensor))
  (let ((result ())
     (tensor-elements (slot-value t1 'elements)))
    (cond ((not (listp (car tensor-elements))) (dolist (it tensor-elements result) (setf result (append result (list (equal-val (slot-value t2 'elements) it))))))
        (t (loop for x in tensor-elements
          do (setf result (append result (list (equal-val-tensor-scalar t2 (make-instance 'tensor :tensor-elements x))))))))
    result))


(defun equal-val-tensor-tensor (t1 t2)
  (let ((result ()))
    (cond ((not (listp (car t1))) (loop for x in t1
                      for y in t2
                     do (setf result (append result (list (equal-val x y))))))
        (t (loop for x in t1
             for y in t2
...
(defgeneric and-val-tensor-scalar (t1 t2))

(defmethod and-val-tensor-scalar ((t1 tensor) (t2 scalar))
  (let ((result ())
     (tensor-elements (slot-value t1 'elements)))
    (cond ((not (listp (car tensor-elements))) (dolist (it tensor-elements result) (setf result (append result (list (and-val it (slot-value t2 'elements)))))))
        (t (loop for x in tensor-elements
          do (setf result (append result (list (and-val-tensor-scalar (make-instance 'tensor :tensor-elements x) t2)))))))
    result))

(defmethod and-val-tensor-scalar ((t2 scalar) (t1 tensor))
  (let ((result ())
     (tensor-elements (slot-value t1 'elements)))
    (cond ((not (listp (car tensor-elements))) (dolist (it tensor-elements result) (setf result (append result (list (and-val (slot-value t2 'elements) it))))))
        (t (loop for x in tensor-elements
          do (setf result (append result (list (and-val-tensor-scalar t2 (make-instance 'tensor :tensor-elements x))))))))
    result))


(defun and-val-tensor-tensor (t1 t2)
  (let ((result ()))
    (cond ((not (listp (car t1))) (loop for x in t1
                      for y in t2
                     do (setf result (append result (list (and-val x y))))))
        (t (loop for x in t1
             for y in t2
          do (setf result (append result (list (and-val-tensor-tensor x y)))))))
    result))